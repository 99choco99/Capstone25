...
namespace UniversalGraph.Generator
{
    ...
    public sealed class DialogueBindingGenerator : ISourceGenerator
    {
        ...
        private const string ProviderInterfaceName =
            "UniversalGraph.IDialogueGeneratedMethodProvider";
        private const string SinkInterfaceName =
            "UniversalGraph.IDialogueGeneratedMethodSink";
        private const string RegistrationTypeName =
            "UniversalGraph.DialogueGeneratedMethodRegistration";
        private const string InvokerTypeName =
            "UniversalGraph.GeneratedMethodInvoker";
        ...
        {
            // Unity loads this analyzer DLL as one unit. Dispatch Quest generation from the same
            // entry point while keeping its runtime-contract checks independent from Dialogue.
            new QuestBindingGenerator().Execute(context);
            ...
            INamedTypeSymbol unityObjectType)
        {
            ...
            if (!hasCorrectReturn)
            {
                ...
                ReportInvalidMethod(context, binding, $"반환 타입은 {expected}여야 합니다.");
                isValid = false;
            }

            if (method.IsAbstract)
            {
                ReportInvalidMethod(context, binding, "abstract 메서드는 호출할 수 없습니다.");
                isValid = false;
            }
            ...
            {
                ReportInvalidMethod(context, binding, "제네릭 메서드와 제네릭 선언 타입은 지원하지 않습니다.");
                isValid = false;
            }

            if (method.IsAsync)
            {
                ReportInvalidMethod(context, binding, "async 메서드는 지원하지 않습니다.");
                isValid = false;
            }
            ...
            {
                ReportInvalidMethod(context, binding, "일반 메서드만 지원합니다.");
                isValid = false;
            }
            ...
            IReadOnlyList<DialogueBinding> bindings)
        {
            ...
            builder.Append("[assembly: global::UniversalGraph.DialogueGeneratedProviderAttribute(typeof(global::UniversalGraph.Dialogue.Generated.")
                .Append(providerName)
                .AppendLine("))]");
            builder.AppendLine();
            ...
            builder.Append("    public sealed class ")
                .Append(providerName)
                .AppendLine(" : global::UniversalGraph.IDialogueGeneratedMethodProvider");
            builder.AppendLine("    {");
            builder.AppendLine("        [global::UnityEngine.Scripting.Preserve]");
            builder.AppendLine("        public void Collect(global::UniversalGraph.IDialogueGeneratedMethodSink sink)");
            builder.AppendLine("        {");

            for (int index = 0; index < bindings.Count; index++)
            {
                DialogueBinding binding = bindings[index];
                builder.AppendLine("            sink.Add(new global::UniversalGraph.DialogueGeneratedMethodRegistration(");
