...
namespace UniversalGraph
{
	public class DialogueManager
	{
		...
		public bool IsConversationActive => (Object)currentContainer != (Object)null;
		...
		public bool TryStartConversation(DialogueContainer container, string entryId, DialogueContext context, Action onComplete = null)
		{
			...
			if (context == null)
			{
				Debug.LogError((object)("[Dialogue] '" + ((Object)container).name + "'???\u0080?붾? ?쒖옉?섎젮硫?DialogueContext媛\u0080 ?꾩슂?⑸땲??"));
				return false;
			}
			if (!container.TryResolveEntry(entryId, out var entryNode, out var error))
			{
				Debug.LogError((object)("[Dialogue] ?\u0080?붾? ?쒖옉?????놁뒿?덈떎. " + error), (Object)container);
				return false;
			}
