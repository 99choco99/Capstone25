...
namespace UniversalGraph.Dialogue.Editor
{
    ...
    public class DialogueStartNode : GraphNode<DialogueStartNodeData>
    {
        public override Vector2 DefaultSize => new(180f, 100f);

        /// <summary>노드가 처음 생성 됐을 때 데이터를 넣어주는 함수</summary>
        protected override void InitializeNewData(DialogueStartNodeData data, GraphNodeDataCreationContext context)
        {
            HashSet<string> usedEntryIds = new(context.ExistingNodes.OfType<DialogueStartNodeData>().Select(entry => entry.EntryId));

            if (!usedEntryIds.Contains(DialogueStartNodeData.DefaultEntryId))
            {
                data.EntryId = DialogueStartNodeData.DefaultEntryId;
                return;
            }
        ...
        public override VisualElement CreateInspector(NodeInspectorEditHandler editHandler)
        {
            ...
            {
                string requestedEntryId = string.IsNullOrWhiteSpace(change.newValue) ? DialogueStartNodeData.DefaultEntryId : change.newValue.Trim();
                UniversalGraphView graphView = GetFirstAncestorOfType<UniversalGraphView>();
                bool isDuplicate = graphView != null
                    && graphView.nodes.OfType<DialogueStartNode>().Any(node => node != this && node.NodeData?.EntryId == requestedEntryId);
