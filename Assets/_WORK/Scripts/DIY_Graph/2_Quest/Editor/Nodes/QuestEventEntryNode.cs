...
namespace UniversalGraph.Quest.Editor
{
    ...
    public sealed class QuestStartNode : GraphNode<QuestStartNodeData>
    {
        ...
        protected override void InitializeNewData(QuestStartNodeData data, GraphNodeDataCreationContext context)
        {
            if (context.ExistingNodes.Any(node => node is QuestStartNodeData))
            {
        ...
        {
            return new HelpBox(
                "Quest 진행은 여기에서 시작합니다. 상호작용 대화 진입점은 별도이며 Quest를 시작하지 않습니다.",
                HelpBoxMessageType.Info);
        }
    }

    /// <summary>플레이어가 프로젝트에서 정의한 대상과 상호작용할 때 대화 경로 탐색에 사용하는 시작점입니다.</summary>
    [GraphNodeEditor(typeof(QuestContainer), "Quest/Entry/Interaction Dialogue")]
    public sealed class QuestEventEntryNode : GraphNode<QuestEventEntryNodeData>
    {
        ...
        public override VisualElement CreateInspector(NodeInspectorEditHandler editHandler)
        {
            ...
            root.Add(new Label("Interaction Dialogue Entry"));
            root.Add(new HelpBox(
                "상호작용 대상에게 제공할 대화를 선택하는 진입점입니다. 모든 대상과 일치시키려면 Target ID를 비워 두세요.",
                HelpBoxMessageType.Info));
