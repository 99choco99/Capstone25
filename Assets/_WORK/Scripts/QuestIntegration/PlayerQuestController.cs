...
public class PlayerQuestController : UniversalGraph.IQuestController
{
    ...
    public List<QuestDialogueContext> GetCurrentQuestContextForNPC(int npcID)
    {
        ...
        foreach (var status in QuestProgress.Values)
        {
            ...
            UniversalGraph.QuestContainer template = QuestManager.Instance.GetQuestTemplate(status.questId);
    ...
    public void UpdateObjectiveProgress(string type, int targetId, int amount)
    {
        ...
        {
            UniversalGraph.QuestContainer template = QuestManager.Instance.GetQuestTemplate(progress.questId);
            if (template == null) { return; }
